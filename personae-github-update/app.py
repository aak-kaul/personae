...
SAMPLE_PATH = os.path.join(_HERE, "sample_data", "CC_GENERAL.csv")
MALL_PATH = os.path.join(_HERE, "sample_data", "Mall_Customers.csv")
...
def analyze(df, k=None):
    ...
    grounding = gc.verify_all(personas, result)   # automated grounding check
...
def index():
    return render_template("index.html",
                           ...
                           has_mall=os.path.exists(MALL_PATH))


@app.route("/analyze", methods=["POST"])
def do_analyze():
    try:
        sample = request.form.get("sample")            # "cc" | "mall" | None
        ...
        if sample == "mall":
            if not os.path.exists(MALL_PATH):
                flash("Mall sample not found on server.")
                return redirect(url_for("index"))
            df = pd.read_csv(MALL_PATH)
            if k is None:
                k = 6          # silhouette-optimal for the Mall dataset
        elif sample == "cc" or use_sample:
