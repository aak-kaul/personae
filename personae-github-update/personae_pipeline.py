...
def _looks_like_id(name):
    ...
    known = {"custid", "id", "customerid", "custno", "customerno",
             "customernumber", "index", "unnamed:0", "rowid", "sno", "srno"}
    return n in known or n.endswith("id")


def select_numeric_features(df):
    """Generalize to arbitrary schemas: keep numeric feature columns, drop
    identifiers. Handles CC_GENERAL (CUST_ID), Mall (CustomerID), and unnamed
    index columns. Also drops any numeric column that is a perfectly unique
    sequential integer (a give-away identifier)."""
    drop = [c for c in df.columns if _looks_like_id(c)]
    ...
    for c in num.columns:
        col = num[c].dropna()
        # sequential unique integer id (e.g., 1..N) -> skip
        is_int = np.array_equal(col.values, col.round().values)
        if (len(col) == n and col.nunique() == n and is_int
                and float(col.max()) - float(col.min()) + 1 == n):
            continue
        feats.append(c)
    return feats if len(feats) >= 2 else list(num.columns)
